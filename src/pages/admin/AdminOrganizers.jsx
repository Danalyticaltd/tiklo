  ...
  return (
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <Link to="/admin" className="flex items-center gap-1.5 text-sm text-muted hover:text-gray-900 mb-6">
          <ArrowLeft size={14} /> Admin dashboard
        </Link>

        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-heading text-2xl font-bold text-gray-900">Organizers</h1>
            {pendingCount > 0 && (
              <p className="text-amber-600 text-sm mt-1">{pendingCount} pending approval</p>
            )}
          </div>
            ...
            {['pending', 'approved', 'all'].map(f => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg border transition capitalize font-medium
                  ${filter === f ? 'bg-primary border-primary text-white' : 'border-gray-200 text-muted hover:text-gray-900'}`}
              >
              ...
              <div key={o.id} className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {o.approved ? (
                    <button
                      onClick={() => revoke(o.id)}
                      className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-muted hover:border-red-400 hover:text-red-500 transition"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg border border-green-500 text-green-600 hover:bg-green-50 transition font-medium"
                    >
